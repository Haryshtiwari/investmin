...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  // Set dark theme when user is authenticated
  useEffect(() => {
    if (state.isAuthenticated && state.user) {
      setTheme('dark');
    }
  }, [state.isAuthenticated, state.user, setTheme]);
  ...
  const login = async (email: string, password: string) => {
    try {
      ...
      if (response.success && response.data) {
        ...
        // Set theme to dark after successful login
        setTheme('dark');
  ...
  const register = async (userData: {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // Set theme to dark after successful registration
        setTheme('dark');
      } else {
