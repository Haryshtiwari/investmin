...
export default function WishlistPanel({ isOpen, onClose, inline = false }: Props) {
                ...
                symbols.map((s) => {
                  ...
                  return (
                    <div key={s} className="group">
                      {/* Main Symbol Row */}
                      <div className="flex items-center p-1.5 rounded-lg hover:bg-gradient-to-r hover:from-gray-700/50 hover:to-black/70 border border-transparent hover:border-gray-600/40 transition-all duration-200 cursor-pointer" 
                           onClick={() => { setSelectedSymbol(s); onClose(); }}>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            <span className="font-bold text-sm text-gray-100 drop-shadow-sm">{s}</span>
                            <span className={`text-xs font-mono ${change >= 0 ? 'text-emerald-400' : 'text-red-400'} drop-shadow-sm`}>
                      ...
                      {/* Bid/Ask and Action Buttons Row */}
                      <div className="flex items-center justify-between px-1.5 py-1 text-xs">
                        <div className="flex items-center gap-2 text-xs">
                          <span className="text-emerald-400">Bid: <span className="font-mono text-gray-300">{fmt(bid)}</span></span>
                          <span className="text-red-400">Ask: <span className="font-mono text-gray-300">{fmt(ask)}</span></span>
                        </div>
                        <div className="flex items-center gap-1">
  ...
  return (
    <div aria-hidden={!isOpen} className={`absolute inset-0 z-[70] ${isOpen ? '' : 'pointer-events-none'}`} role="dialog" aria-modal={isOpen}>
      ...
      <div className={`absolute right-0 transition-transform duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`} style={{ width: '30%', top: '0', height: '100%' }}>
        <Card className="flex flex-col bg-gradient-to-br from-gray-900/95 to-black/90 backdrop-blur-md border-l border-gray-600/50 h-full shadow-2xl"
              style={{
                boxShadow: "inset 2px 2px 8px rgba(0,0,0,0.4), inset -2px -2px 8px rgba(255,255,255,0.08), 0 12px 40px rgba(0,0,0,0.3)"
              }}>
